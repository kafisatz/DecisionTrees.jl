warn("BK:Possondeviancesplit -  this is in the works...")
...
warn("xlogy will fail terribly if one of the arguments is zero! we need to amend the code to handle that case properly!")
  ...
  for i in subs
      ...
      if (sumwl>minweight)&&(weighttot_minw>sumwl)
        #deviancel[i],deviancer[i]=get_poisson_deviances(meansl[counter],meansr[counter],lo,ooo,f::pdaMod,numerator::Array{Float64,1},denominator::Array{Float64,1},weight::Array{Float64,1},elementsInLeftChildBV)
        deviancel,deviancer=get_poisson_deviances(meansl[counter],meansr[counter],lo,ooo,f::pdaMod,numerator::Array{Float64,1},denominator::Array{Float64,1},weight::Array{Float64,1},elementsInLeftChildBV)
      #end
...
function get_poisson_deviances(current_meanl::Float64,current_meanr::Float64,lo,ooo,f::pdaMod,numerator::Array{Float64,1},denominator::Array{Float64,1},weight::Array{Float64,1},elementsInLeftChildBV)
  ...
  for count=1:length(a)
		 @inbounds idx=a[count] + ooo #idx will be the UInt8 label of the observation
     @inbounds ni = numerator[count]
     if elementsInLeftChildBV[idx]
       dl += (xlogy(ni, ni / current_meanl) - (ni - current_meanl))
     else
       dr += (xlogy(ni, ni / current_meanr) - (ni - current_meanr))
     end
   end
  return dl,dr
end
