time_x_started=now()
#this_dir="C:\\Users\\bernhard.konig\\Documents\\ASync\\irobot\\TariffWatch New Repo\\algorithms\\Julia\\Code\\dev\\src";
this_dir="C:\\Users\\bernhard.konig\\async\\TariffWatch New Repo\\algorithms\\Julia\\Code\\dev\\src"
cd(this_dir);unshift!(LOAD_PATH,this_dir)
using DTM
using DataFrames
...
#folder and dataset
    ...
    #tmpf=string(rootfolder,"China Guanjun Lead\\Working Files\\data\\data20k.csv")
	#datafolder=string() #","\\China Guanjun Lead\\Working Files\\preliminary_models_privatecars_only_wo_glass\\") #C:\\temp\\"
	tmpf=string("C:\\Users\\bernhard.konig\\async\\data","\\data20k.csv")
	datafolder="c:\\temp\\"
...
#expl_cat=vcat(expl_cat,["COV_NAME"]) #excluding cov_name as we do not want it as a predictor
    ...
    expl_cat=["PROD_CODE" "BRAND_NAME_JY" "ORG_CLASS" "CUST_TYPE" "CHANNEL_GROUP" "NCD_BFD" "IS_NEW" "MULTI_COV" "TRUCK_TYPE" "SERIES_LEVEL" "GRP_SEX" "GRP_RENEW" "GRP_NON_DEDUC"][:];
...
#prep df
@time (df_prepped,this_sett)=prepare_dataframe_for_dtm!(df_tmp,treat_as_categorical_variable=forced_categorical_variables,trnvalcol=chosen_trn_val_col,weightcol=chosen_weight_col,numcol=chosen_num_col,denomcol=chosen_denom_col,independent_vars=expl);
...
@time updateSettingsMod!(this_sett,model_type="build_tree",write_iteration_matrix="false",minw=-.2,randomw=0.0,mf=.1,subsampling_features_prop=1,niter=50,subsampling_prop=1.00,graphvizexecutable="C:\\Program Files (x86)\\Graphviz2.38\\bin\\dot.exe",write_result=false)
outfileStringOnly="tree"
outfilename=string(datafolder,outfileStringOnly)
dataFilename=string(datafolder,"prepped.jld")

warn("BK: possibly exclude Tonnage, PREFER to exclude Amount as it is per coverage!!!")
...
df_prepped=DataFrame(); gc(); #try to save some memeory by 'deleting df_prepped'
