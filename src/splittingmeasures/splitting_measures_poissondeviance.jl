...
tmp_result=calculateSplitValue(crit,labellist,sumnumerator,sumdenominator,sumweight,countlistfloat,minweight,subs,numerator,denominator,weight,features)

	# labellist_sorted # we do not need that do we?
...
#warn("xlogy will fail terribly if one of the arguments is zero! we need to amend the code to handle that case properly!")
  ...
  (lo::UInt8, hi::UInt8) = extrema(levels(features)) #(lo, hi) = extrema(levels(f))
  ooo=one(lo)-lo
  vecsize=hi+ooo
