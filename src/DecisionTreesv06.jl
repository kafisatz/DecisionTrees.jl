...
using OnlineStats, StatsBase, StatsFuns, DataFrames, ProgressMeter, PyCall, FileIO, JLD2 # DataArrays is not used anymore
...
function get_sha1()
	which_res=@which DecisionTreesv06.__init__()
	dtpath=splitdir(splitext(string(which_res.file))[1])[1][1:end-4]
	current_path=pwd()
	cd(dtpath)
	cmd=`git rev-parse HEAD`
	#txa=run(cmd)
    sha1_of_dt="uninitialized"
    try
